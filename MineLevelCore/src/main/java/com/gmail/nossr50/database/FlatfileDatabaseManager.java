...
public final class FlatfileDatabaseManager implements DatabaseManager {
    ...
    public void saveAll(List<PlayerProfile> profiles) {
        List<PlayerProfile> profilesCopy = new ArrayList();
        for(PlayerProfile profilec : profiles){
            profilec.changed=false;
            profilesCopy.add(new PlayerProfile(profilec.playerName, profilec.uuid, ImmutableMap.copyOf(profilec.skills), ImmutableMap.copyOf(profilec.skillsXp), ImmutableMap.copyOf(profilec.abilityDATS), profilec.mobHealthbarType, profilec.scoreboardTipsShown));
        }
        synchronized (fileWritingLock){
            BufferedReader in = null;
            FileWriter out = null;
            try {
                ...
                String line;
                    ...
                    while ((line = in.readLine()) != null) {
                        ...
                        String[] character = line.split(":");
                        for (PlayerProfile profile : profilesCopy) {
                                if(profile.getUniqueId() != null && character[UUID_INDEX].equalsIgnoreCase(profile.getUniqueId().toString()) && character[USERNAME].equalsIgnoreCase(profile.getPlayerName())) {
                                    ...
                                    writer.append(profile.getUniqueId() != null ? profile.getUniqueId().toString() : "NULL").append(":");
