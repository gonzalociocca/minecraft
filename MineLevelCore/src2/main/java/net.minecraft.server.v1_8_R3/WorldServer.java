...
public class WorldServer extends World implements IAsyncTaskHandler {
    ...
    private List<NextTickListEntry> V = new ConcurrentArrayList<>();
    ...
    public TileEntity getTileEntity(BlockPosition pos) {
        TileEntity result = this.getTileEntity(pos,0);
        return result;}
    @Override
    public TileEntity getTileEntity(BlockPosition pos, int run) {
        TileEntity result = super.getTileEntity(pos,run);
    ...
    private TileEntity fixTileEntity(BlockPosition pos, Block type, TileEntity found) {
        ...
        if (type instanceof IContainer) {
            ...
            this.setTileEntity(pos, replacement,0);
            return replacement;
        } else {
    ...
    public void doTick() {
        ...
        if (getGameRules().getBoolean("doMobSpawning") && worldData.getType() != WorldType.DEBUG_ALL_BLOCK_STATES && (allowMonsters || allowAnimals) && (WorldServer.this instanceof WorldServer && players.size() > 0)) {
            timings.mobSpawn.startTiming(); // Spigot
 //           int maxsize = players.size()*60;
 //   spawnMobs(maxsize > entityList.size() && allowMonsters && ticksPerMonsterSpawns != 0L, maxsize > entityList.size() && allowAnimals && ticksPerAnimalSpawns != 0);
        ...
        if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
            ...
            boolean next;
                ...
                this.V.clear();
                    ...
                    synchronized (M){return !this.M.isEmpty();}
    ...
    public void sendParticles(EntityPlayer sender, EnumParticle enumparticle, boolean flag, double d0, double d1, double d2, int i, double d3, double d4, double d5, double d6, int... aint) {
        ...
        for (EntityHuman entityplayer : players) {
            ...
            if (d7 <= 256.0D || flag && d7 <= 65536.0D) {
                ((EntityPlayer)entityplayer).playerConnection.sendPacket(packetplayoutworldparticles);
            }
        }
