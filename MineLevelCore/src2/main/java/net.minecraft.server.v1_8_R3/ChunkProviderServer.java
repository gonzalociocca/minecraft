...
public class ChunkProviderServer implements IChunkProvider {
    ...
    //testpublic ConcurrentHashMap<Long,Chunk> chunks = new ConcurrentHashMap();
    ...
    public boolean isChunkLoaded(int i, int j) {
       synchronized (chunks){return this.chunks.containsKey(LongHash.toLong(i, j)); // CraftBukkit
    }}

    // CraftBukkit start - Change return type to Collection and return the values of our chunk map
    public java.util.Collection a() {
        // return this.chunkList;
        synchronized (world.chunkProviderServer.chunks){ return this.chunks.values();}
        // CraftBukkit end
    }
    ...
    public Chunk getChunkIfLoaded(int x, int z) {
        synchronized (chunks){return chunks.get(LongHash.toLong(x, z));}
    }
    ...
    public Chunk getChunkAt(int i, int j, Runnable runnable) {
        ...
        Chunk chunk;
        synchronized (chunks){
         chunk = chunks.get(LongHash.toLong(i, j));}
    ...
    public Chunk originalGetChunkAt(int i, int j) {
        ...
        Chunk chunk;
                synchronized (chunks){chunk = (Chunk) this.chunks.get(LongHash.toLong(i, j));}
...
synchronized (chunks){
    ...
    public void getChunkAt(IChunkProvider ichunkprovider, int i, int j) {
        ...
        if (!chunk.isDone()) {
            chunk.n();
            if (this.chunkProvider != null) {
                try{this.chunkProvider.getChunkAt(ichunkprovider, i, j);}catch(Exception e){
                    System.out.println("Already decorating at ChunkProviderServer.getChunkAt, preventing crash");
                }
...
try{
        Iterator iterator;
        synchronized (chunks){iterator = this.chunks.values().iterator();}
        while (iterator.hasNext()) {
            Chunk chunk;
        ...
        if (!this.world.savingDisabled) {
            ...
            for (int i = 0; i < 100 && !this.unloadQueue.isEmpty(); ++i) {
                long chunkcoordinates = this.unloadQueue.popFirst();
                 Chunk chunk;synchronized (world.chunkProviderServer.chunks){ chunk = this.chunks.get(chunkcoordinates);}
    ...
    public String getName() {
        ...
        synchronized (world.chunkProviderServer.chunks){  return "ServerChunkCache: " + this.chunks.size() + " Drop: " + this.unloadQueue.size();}
    }
    ...
    public int getLoadedChunks() {
        ...
        synchronized (world.chunkProviderServer.chunks){return this.chunks.size();}
    }
