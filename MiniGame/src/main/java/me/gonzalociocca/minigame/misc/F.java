...
public class F {
    ...
    public static String help(String cmd, String body, Rank rank) {
        return rank.getRankType().getChatPrefix() + cmd + " " + C.mBody + body + " " + rank(rank);
    }

    public static String rank(Rank rank) {
        if (!rank.getRankType().isAtLeast(Rank.RankType.VIP)) {
            return rank.getRankType().getColor() + "Player";
        }

        return rank.getRankType().getChatPrefix();
    }
