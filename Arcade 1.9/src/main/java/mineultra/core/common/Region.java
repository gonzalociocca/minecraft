...
public class Region
{
  ...
  {
    if ((v.getBlockX() >= _minX) && (v.getBlockX() <= _maxX) && 
  ...
  {
    _blockPassage = Boolean.valueOf(!canEnter.booleanValue());
  }

  public void SetChangeBlocks(Boolean canChangeBlocks)
  {
    _blockChange = Boolean.valueOf(!canChangeBlocks.booleanValue());
  }

  public Boolean CanEnter(String playerName)
  {
    if (_blockPassage.booleanValue())
    {
