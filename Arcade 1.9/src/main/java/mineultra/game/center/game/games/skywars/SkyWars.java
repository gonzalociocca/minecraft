...
this.AddGems(p, 25.0D, "Ganar el juego!", true);
         }
   else if(p.isOnline()){
...
try{
  SetCustomWinLine(C.cDGreen+"El jugador "+ChatColor.RED+""+ChatColor.BOLD+WinnerTeam.GetPlayers(false).get(0).getName()+ C.cDGreen + " ha ganado el juego!");
}catch(Exception e){
 ...
 public void onEyeTeleport(PlayerInteractEvent event){
    if(event.getAction() == Action.LEFT_CLICK_AIR || event.getAction() == Action.LEFT_CLICK_BLOCK || event.getAction() ==  Action.PHYSICAL){
     ...
     event.setCancelled(true);
         if(!nextlocs.containsKey(p)){
             p.sendMessage(Colorizer.Color("&eIntenta en unos segundos."));
return;
         }
         Location loc = prevlocs.get(p);
        ((CraftPlayer)p).getHandle().enderTeleportTo(loc.getX(), loc.getY(), loc.getZ());
        p.sendMessage(Colorizer.Color("&aTeletransportado a &eX: "+loc.getX()+" / Y: "+loc.getY()+" / Z: "+loc.getZ()));
...
this.GetObjectiveSide().getScore(Colorizer.Color("&fwww.mineultra.com")).setScore(-2);
...
if(finishtime < System.currentTimeMillis()){
  for(GameTeam gt : this.GetTeamList()){
    ...
    if(p.isOnline()){
p.sendMessage(Colorizer.Color("&eEl juego ha sido muy largo, por AFKs, perdona."));
this.AddGems(p, 10.0D, "Participacion", true);
        db.getPlayerData(p).addLoss(1);
    ...
    this.Manager.GetGame().time3 = 0;
       ...
       for(Player p : Bukkit.getOnlinePlayers()){
        ...
        if(alertTime< System.currentTimeMillis()){
         pe.sendMessage(Colorizer.Color("&aEl juego terminara en "+(finishtime-System.currentTimeMillis())/1000+"segundos&e, Razon: AFKs"));
     }
    }
