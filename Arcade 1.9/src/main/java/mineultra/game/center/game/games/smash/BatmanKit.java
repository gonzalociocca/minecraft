...
{
    public BatmanKit(final centerManager manager) {
        super(manager, "Batman", KitAvailability.Green, new String[] { "Batman return`s..!" }
                , new Perk[] {new BATWAVE(),new SMASH("Smash",1,2500L) }
...
ItemStack eye = new ItemStack(Material.COAL);
ItemMeta ime = eye.getItemMeta();
ime.setDisplayName(ChatColor.RED+""+ChatColor.BOLD+"BatWave");
eye.setItemMeta(ime);
...
return Color.BLACK;
    ...
    @Override
    public int GetCost() {
        return this.Manager.getConfig().getInt("kitprices.batman");
    }
}
