...
{
    public WitherKit(final centerManager manager) {
        super(manager, "Wither", KitAvailability.Green, new String[] { "Launch wither heads" }
                , new Perk[] {new WITHERSKULL(),new SMASH("Smash",1,2500L) }
                , EntityType.PLAYER, new ItemStack(Material.WOOD_SWORD));
    }
...
ime.setDisplayName(ChatColor.RED+""+ChatColor.BOLD+"SpiderFood");
eye.setItemMeta(ime);
...
return Color.BLACK;
    ...
    @Override
    public int GetCost() {
        return this.Manager.getConfig().getInt("kitprices.wither");
    }
}
