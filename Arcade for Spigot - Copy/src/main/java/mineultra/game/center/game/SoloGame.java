...
public abstract class SoloGame extends Game
{
    ...
    public void EndCheck() {
        ...
        if (this.GetPlayers(true).size() <= 0) {
            ...
            if (this._places.size() >= 1) {
                this.AddGems(this._places.get(0), 20.0, "1st place", false);
            }
            if (this._places.size() >= 2) {
                this.AddGems(this._places.get(1), 15.0, "2nd place", false);
            }
            if (this._places.size() >= 3) {
                this.AddGems(this._places.get(2), 10.0, "3rd place", false);
            }
    ...
    public void ScoreboardUpdate(final UpdateEvent event) {
        ...
        this.GetObjectiveSide().getScore(team.GetColor() + "Alive").setScore(team.GetPlayers(true).size());
        this.GetObjectiveSide().getScore(String.valueOf(C.cRed) + "Dead").setScore(team.GetPlayers(false).size() - team.GetPlayers(true).size());
    }
