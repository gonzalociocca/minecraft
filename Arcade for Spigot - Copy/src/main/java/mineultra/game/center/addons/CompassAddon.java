...
public class CompassAddon extends MiniPlugin
{
            ...
            if (this.Manager.GetGame().CompassEnabled || !this.Manager.GetGame().IsAlive(player)) {
                ...
                if (target != null) {
                    ...
                    for (final int i : player.getInventory().all(Material.COMPASS).keySet()) {
                        ...
                        itemMeta.setDisplayName("    " + C.cWhite + C.Bold + "Nearest Player: " + targetTeam.GetColor() + target.getName() + "    " + C.cWhite + C.Bold + "Distance: " + targetTeam.GetColor() + UtilMath.trim(1, bestDist) + "    " + C.cWhite + C.Bold + "Height: " + targetTeam.GetColor() + UtilMath.trim(1, heightDiff));
    ...
    public void DropItem(final PlayerDropItemEvent event) {
        ...
        UtilPlayer.message((Entity)event.getPlayer(), F.main("Game", "You cannot drop " + F.item("Target Compass") + "."));
    }
