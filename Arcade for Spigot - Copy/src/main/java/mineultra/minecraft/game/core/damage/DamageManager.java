...
import mineultra.game.center.center;
...
public class DamageManager extends MiniPlugin
{
    ...
    public DamageManager(final JavaPlugin plugin, centerManager manager,final CombatManager combatManager, final NpcManager npcManager) {
        ...
        try {
            (this._lastDamageByPlayerTime = EntityLiving.class.getDeclaredField("lastDamageByPlayerTime")).setAccessible(true);
            (this._k = EntityLiving.class.getDeclaredMethod("h", Float.TYPE)).setAccessible(true);
        }
   ...
   /*insert disguise sounds here*/
    ...
    public Float damagevalue(EntityLiving pe){
      ...
      Float value = 2F;
      try{
        if(pe.getEquipment(0) != null){
  ItemStack helmet = CraftItemStack.asNewCraftStack(pe.getEquipment(0).getItem());
...
if(helmet.containsEnchantment(Enchantment.PROTECTION_ENVIRONMENTAL)){
    value += helmet.getEnchantmentLevel(Enchantment.PROTECTION_ENVIRONMENTAL);
        ...
        if(pe.getEquipment(1) != null){
  ItemStack helmet = CraftItemStack.asNewCraftStack(pe.getEquipment(1).getItem());
value += this.itemvalue(helmet);
if(helmet.containsEnchantment(Enchantment.PROTECTION_ENVIRONMENTAL)){
    value += helmet.getEnchantmentLevel(Enchantment.PROTECTION_ENVIRONMENTAL);
}
         }
        if(pe.getEquipment(2) != null){
  ItemStack helmet = CraftItemStack.asNewCraftStack(pe.getEquipment(2).getItem());
value += this.itemvalue(helmet);
if(helmet.containsEnchantment(Enchantment.PROTECTION_ENVIRONMENTAL)){
    value += helmet.getEnchantmentLevel(Enchantment.PROTECTION_ENVIRONMENTAL);
}
         }
        if(pe.getEquipment(3) != null){
  ItemStack helmet = CraftItemStack.asNewCraftStack(pe.getEquipment(3).getItem());
value += this.itemvalue(helmet);
if(helmet.containsEnchantment(Enchantment.PROTECTION_ENVIRONMENTAL)){
    value += helmet.getEnchantmentLevel(Enchantment.PROTECTION_ENVIRONMENTAL);
}
