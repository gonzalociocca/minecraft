...
public class CraftChunk implements Chunk {
    ...
    public CraftChunk(net.minecraft.server.v1_8_R3.Chunk chunk) {
        if (!(chunk instanceof EmptyChunk)) {
            this.weakChunk = new WeakReference<net.minecraft.server.v1_8_R3.Chunk>(chunk);
        }
    ...
    public net.minecraft.server.v1_8_R3.Chunk getHandle() {
        net.minecraft.server.v1_8_R3.Chunk c = weakChunk.get();

        if (c == null) {
            c = worldServer.getChunkAt(x, z);

            if (!(c instanceof EmptyChunk)) {
                weakChunk = new WeakReference<net.minecraft.server.v1_8_R3.Chunk>(c);
            }
        }
