...
public final class Bukkit {
    ...
    public static Player[] _INVALID_getOnlinePlayers() {
        return server.getOnlinePlayers().toArray(new Player[server.getOnlinePlayers().size()]);
    }
    ...
    public static boolean isPrimaryThread() {
        //MineLevel - Always true
        for(World w : Bukkit.getWorlds()){
            if(w.getName().equalsIgnoreCase(Thread.currentThread().getName())){
                return true;
            }
        }
