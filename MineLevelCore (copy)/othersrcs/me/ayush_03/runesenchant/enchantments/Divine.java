...
public class Divine implements Listener {
   ...
   public void onDamage(EntityDamageByEntityEvent entitydamagebyentityevent) {
      String s = FileManager.getManager().getEnchantments().getString("divine");
      ...
      if(entitydamagebyentityevent.getDamager() instanceof Player && entitydamagebyentityevent.getEntity() instanceof Player) {
         try {
            ...
            if(player.getItemInHand().getItemMeta().getLore().contains(s + " I") && i < Main.getConfiguration().getInt("divine.level_I.chance")) {
               if(player.getHealth() + entitydamagebyentityevent.getDamage() > 20.0D) {
            ...
            if(player.getItemInHand().getItemMeta().getLore().contains(s + " II") && i < Main.getConfiguration().getInt("divine.level_II.chance")) {
               if(player.getHealth() + entitydamagebyentityevent.getDamage() > 20.0D) {
                  player.setHealth(20.0D);
               } else {
                  player.setHealth(player.getHealth() + entitydamagebyentityevent.getDamage());
            ...
            if(player.getItemInHand().getItemMeta().getLore().contains(s + " III") && i < Main.getConfiguration().getInt("divine.level_III.chance")) {
               if(player.getHealth() + entitydamagebyentityevent.getDamage() > 20.0D) {
                  player.setHealth(20.0D);
               } else {
                  player.setHealth(player.getHealth() + entitydamagebyentityevent.getDamage());
