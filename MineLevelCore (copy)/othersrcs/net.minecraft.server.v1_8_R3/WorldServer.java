...
public class WorldServer extends World implements IAsyncTaskHandler {
    ...
    //GonzaloCiocca created Spawnmobs and getRandomEnt Method.
    public void spawnMobs(boolean monsters, boolean animals){
        if(players.size() <= 0){
            return;
        }
        double tickpercent = (random.nextInt(9)+1);
        //1000, tickpercent=5, ticked= 500-599
        double tickedsize = (double)players.size();
        int tickedcount = 0;
        int integera = (int)(tickedsize * (tickpercent / 10));
        if(tickedsize%10 > 0){
            integera+=1;
        }
        int integerb = integera+(int)(tickedsize * (1 / 10));

        for(EntityHuman player : this.players){
            tickedcount++;
            if(tickedcount < integera){
                continue;
            }
            if(tickedcount > integerb){
                break;
            }

            BlockPosition loc = R.getRandomPosition(this, player.getChunkCoordinates().getX(), player.getChunkCoordinates().getZ());


            Long time = this.getWorld().getTime();
            boolean day = time>12300 && time <23850 ? false:true;
            ...
            if(worldProvider instanceof WorldProviderHell && loc.getY() > 128){
                ...
                if(loc.getY() > (this.getHighestBlockYAt(loc).getY()+1)){
                    loc = new BlockPosition(loc.getX(),this.getHighestBlockYAt(loc).getY()+1,loc.getZ());
                }
                if(loc.getY() < 10){
                    continue;
                }
                if (!this.getType(loc).getBlock().u()) {
                    if(Bukkit.getWorld(this.getWorld().getName()).getBlockAt(loc.getX(),loc.getY()-2,loc.getZ()).isLiquid()){
                        this.addEntity(this.getWorld().createEntity(new Location(Bukkit.getWorld(this.getWorld().getName()), loc.getX(), loc.getY(), loc.getZ()),org.bukkit.entity.Squid.class), CreatureSpawnEvent.SpawnReason.NATURAL);
                    }else{
                        this.addEntity(this.getWorld().createEntity(new Location(Bukkit.getWorld(this.getWorld().getName()), loc.getX(), loc.getY(), loc.getZ()),rencl), CreatureSpawnEvent.SpawnReason.NATURAL);
                    }
//                    this.getWorld().spawn(new Location(Bukkit.getWorld(this.getWorld().getName()), loc.getX(), loc.getY(), loc.getZ()), rencl, CreatureSpawnEvent.SpawnReason.NATURAL);
                }}
        }
    }
    ...
    //Gonzalo Ciocca Modified doTick Method.
    public void doTick() {
        super.doTick();
        ...
        if (this.getGameRules().getBoolean("doMobSpawning") && this.worldData.getType() != WorldType.DEBUG_ALL_BLOCK_STATES && (this.allowMonsters || this.allowAnimals) && (this instanceof WorldServer && this.players.size() > 0)) {
            ...
            int maxsize = players.size()*30<3000 ? players.size()*50 : 4000;
    ...
    //Gonzalo Ciocca Modified Method H
    //Gonzalo Ciocca created method isAgeable
    ...
    protected void h() {
        ...
        if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
            ...
            for (gnu.trove.iterator.TLongShortIterator iter = chunkTickList.iterator(); iter.hasNext(); )
            {
                ...
                //grow every 1second now, not 90
                //full crop every 20tick
                if (i1 > 0 && tick==0) {
                    ...
                    for (int k1 = 0; k1 < j1; ++k1) {
                        ...
                        if ((chunksection != null) && (chunksection.shouldTick())) {
                            ...
                            //1000, tickpercent=5, ticked= 500-599
                            double tickedsize = (double)chunksection.tickedblocks.size();
                            ...
                            for(BlockPosition block : chunksection.tickedblocks){
                                ...
                                if(update.isTicking() && update != null && update != Blocks.AIR){
                                    ...
                                    BlockPosition worldposition = new BlockPosition(block.getX()+k,block.getY()+chunksection.getYPosition(),block.getZ()+l);
                                    try{
    ...
    //reducir velocidad tick bloques/redstone
    //a la mitad
    //y redirigir la carga en partes.
    //M=200 cada tick
    //M=100 cada tick?

    // Gonzalo Ciocca modified method a(boolean flag)
    public boolean a(boolean flag) {
        if(M.isEmpty() && V.isEmpty()){
            return false;
        }
        /*not needed        if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
            return false;
        }*/
        /*if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
            return false;
    ...
    protected IChunkProvider k() {
        ...
        // CraftBukkit start
        org.bukkit.craftbukkit.v1_8_R3.generator.InternalChunkGenerator gen;

        if (this.generator != null) {
            gen = new org.bukkit.craftbukkit.v1_8_R3.generator.CustomChunkGenerator(this, this.getSeed(), this.generator);
        } else if (this.worldProvider instanceof WorldProviderHell) {
            gen = new org.bukkit.craftbukkit.v1_8_R3.generator.NetherChunkGenerator(this, this.getSeed());
        } else if (this.worldProvider instanceof WorldProviderTheEnd) {
            gen = new org.bukkit.craftbukkit.v1_8_R3.generator.SkyLandsChunkGenerator(this, this.getSeed());
        } else {
            gen = new org.bukkit.craftbukkit.v1_8_R3.generator.NormalChunkGenerator(this, this.getSeed());
        }
