...
public abstract class MinecraftServer implements Runnable, ICommandListener, IAsyncTaskHandler, IMojangStatistics {
    ...
    public Set<ConcurrentThread> concurrentThreads;
    public Set<ConcurrentCatchupThread> catchupThreads;
    private long ab = az();
    private int threads;
    ...
    public MinecraftServer(OptionSet options, Proxy proxy, File file1) {
        this.concurrentThreads = Sets.newConcurrentHashSet();
        this.catchupThreads = Sets.newConcurrentHashSet();
        //io.netty.util.ResourceLeakDetecctor.setEnabled( false ); // Spigot - disable
        this.e = proxy;
...
/*
        for(int a = 0; a < 1;a++){
            ConcurrentCatchupThread cthread = new ConcurrentCatchupThread(this,a);
            cthread.setName("ConcurrentCatchupThread - "+a);
            cthread.setDaemon(true);
            cthread.start();
            this.catchupThreads.add(cthread);
        }*/
    }
    ...
    protected void a(String s, String s1, long i, WorldType worldtype, String s2) {
        ...
        */
        ...
        for (int j = 0; j < worldCount; ++j) {
            ...
            //MineLevel - Starting to MultiThread WorldServer Entities
            System.out.println("Loading custom world for "+world.getWorld().getName());
            this.createConcurrentThread(world);
        ...
        for (int m = 0; m < worlds.size(); m++) {
            ...
            for (int k = -192; k <= 192 && this.isRunning(); k += 16) {
                for (int l = -192; l <= 192 && this.isRunning(); l += 16) {
                    ...
                    if (i1 - j > 1000L) {
                        this.a_(worldserver.getWorld().getName()+" - Preparing spawn area", i * 100 / 625);
                        j = i1;
                    }
    ...
    int runs = 0;
    Long tpsrecord = System.currentTimeMillis();
            ...
            if (this.init()) {
                ...
                while (this.isRunning) {
                    ...
                    if(runs%1000==0){
                        ...
                        this.tickspersecond = 20.00D-(left/2500.00D);
                        if(tickspersecond<=0){
                            tickspersecond=-1;
                        }
                        System.out.println(serverThread.getName()+" run: "+runs+" tps: "+tickspersecond);
                        tpsrecord = System.currentTimeMillis();
                    }
                    currentTick++;
    ...
    public void createConcurrentThread(WorldServer world){
        this.threads+=1;
        if(world.getWorld().getName().equalsIgnoreCase("world")){
            ConcurrentThread thread = new ConcurrentThread(this,world,true,false,false);
            thread.setName(world.getWorld().getName()+" - Tiles");
            thread.setDaemon(true);
            thread.start();
            this.concurrentThreads.add(thread);

            ConcurrentThread thread2 = new ConcurrentThread(this,world,false,true,true);
            thread2.setName(world.getWorld().getName()+" - Entity");
            thread2.setDaemon(true);
            thread2.start();
            this.concurrentThreads.add(thread2);
        } else{
        ConcurrentThread thread = new ConcurrentThread(this,world,true,true,true);
        thread.setName(world.getWorld().getName()+" - World");
        thread.setDaemon(true);
        thread.start();
        this.concurrentThreads.add(thread);}
    }
  ...
  /*not in use, only adds extra work.
...
*/
    ...
    protected void A() throws ExceptionWorldConflict { // CraftBukkit - added throws
        ...
        /*MineLevel moved to concurrenthread
        if (autosavePeriod > 0 && this.ticks % autosavePeriod == 0) { // CraftBukkit
            SpigotTimings.worldSaveTimer.startTiming(); // Spigot
            this.methodProfiler.a("save");
            //moved to concurrent thread minelevel this.v.savePlayers();
            // Spigot Start
            ...
            //MineLevel - Replace save(false) with parallelStream
            //worlds.parallelStream().forEach(wr -> wr.getWorld().save(false));
             moved to ConcurrentThread minelevel
    ...
    public void B() {
        ...
        if (this.ticks % 20 == 0) {
            for (EntityPlayer entityplayer : this.getPlayerList().v()) {
        ...
        for (i = 0; i < this.worlds.size(); ++i) {
            ...
            // if (i == 0 || this.getAllowNether()) {
                ...
                if(this.concurrentThreads.isEmpty()){
                try {
                ...
                }}else if(threads < worlds.size()){
                    //MineLevel - Creating concurrent threads on default world loader
                    Set<WorldServer> wws = Sets.newConcurrentHashSet();
                    for(WorldServer ws : worlds){
                        wws.add(ws);
                    }

                    for(ConcurrentThread ts : concurrentThreads){
                        for(WorldServer s : wws){
                            if(ts.getWorldServer().getWorld().getName().equalsIgnoreCase(s.getWorld().getName())){
                                wws.remove(s);
                            }
                        }
                    }
