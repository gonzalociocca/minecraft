...
public class TileEntityChest extends TileEntityContainer implements IUpdatePlayerListBox, IInventory {
    ...
    public void m(int run) {
        if (!this.a) {
            this.a = true;
            this.h = this.a(EnumDirection.WEST,run);
            this.g = this.a(EnumDirection.EAST,run);
            this.f = this.a(EnumDirection.NORTH,run);
            this.i = this.a(EnumDirection.SOUTH,run);
        }
    }

    protected TileEntityChest a(EnumDirection enumdirection) {
return this.a(enumdirection,0);
    }
    protected TileEntityChest a(EnumDirection enumdirection,int run) {
        BlockPosition blockposition = this.position.shift(enumdirection);

        if (this.b(blockposition)) {
            TileEntity tileentity = this.world.getTileEntity(blockposition,run);
        ...
        if (!entityhuman.isSpectator()) {
            ...
            if (this.w() == Blocks.TRAPPED_CHEST) {
                ...
                if (oldPower != newPower) {
                    org.bukkit.craftbukkit.v1_8_R3.event.CraftEventFactory.callRedstoneChange(world, position.getX(), position.getY(), position.getZ(), oldPower, newPower);
                }
            }
            // CraftBukkit end
