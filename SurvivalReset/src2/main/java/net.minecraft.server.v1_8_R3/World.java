...
import org.bukkit.craftbukkit.v1_8_R3.SpigotTimings; // Spigot
...
public abstract class World implements IBlockAccess {
    ...
    public final List<Entity> entityList = new ConcurrentArrayList<Entity>();
    // Spigot end
    protected final List<Entity> g = new ConcurrentArrayList();
    ...
    public final List<Entity> k = new ConcurrentArrayList();
        ...
        if (this.captureTreeGeneration) {
            ...
            for(BlockState previous : capturedBlockStates) {
                ...
                if (previous.getX() == blockposition.getX() && previous.getY() == blockposition.getY() && previous.getZ() == blockposition.getZ()) {
                    blockstate = previous;
                   capturedBlockStates.remove(previous);
                    break;
                }
            }
            if (blockstate == null) {
    ...
    public void update(BlockPosition blockposition, Block block) {
        if (this.worldData.getType() != WorldType.DEBUG_ALL_BLOCK_STATES) {
            ...
            // CraftBukkit end
            this.applyPhysics(blockposition, block,0);
        }
    ...
    public void applyPhysics(BlockPosition blockposition, Block block) {
        ...
        this.d(blockposition.west(), block,0);
        this.d(blockposition.east(), block,0);
        this.d(blockposition.down(), block,0);
        this.d(blockposition.up(), block,0);
        this.d(blockposition.north(), block,0);
        this.d(blockposition.south(), block,0);
        spigotConfig.antiXrayInstance.updateNearbyBlocks(this, blockposition); // Spigot
    }

    public void applyPhysics(BlockPosition blockposition, Block block,int limit) {
        limit++;
        this.d(blockposition.west(), block,limit);
        this.d(blockposition.east(), block,limit);
        this.d(blockposition.down(), block,limit);
        this.d(blockposition.up(), block,limit);
        this.d(blockposition.north(), block,limit);
        this.d(blockposition.south(), block,limit);
        spigotConfig.antiXrayInstance.updateNearbyBlocks(this, blockposition); // Spigot
    }
    ...
    public void d(BlockPosition blockposition, final Block block) {
        if (!this.isClientSide) {
            ...
            try {
                ...
                // CraftBukkit end
                if(iblockdata.getBlock() instanceof BlockRedstoneWire){
                    ((BlockRedstoneWire)iblockdata.getBlock()).doPhysics(this,blockposition,iblockdata,block,0);
                ...
                System.out.println("Removing redstone at: "+ blockposition.getX() + ", " + blockposition.getY() + ", " + blockposition.getZ()); // Spigot End
            } catch(Exception e){
                e.printStackTrace();
            }
        }
    ...
    public void d(BlockPosition blockposition, final Block block,int limit) {
        if (!this.isClientSide) {
            limit++;
            IBlockData iblockdata = this.getType(blockposition);
            ...
            try {
                ...
                // CraftBukkit end
                if(iblockdata.getBlock() instanceof BlockRedstoneWire){
                    ((BlockRedstoneWire)iblockdata.getBlock()).doPhysics(this,blockposition,iblockdata,block,limit);
    ...
    public List<AxisAlignedBB> getCubes(Entity entity, AxisAlignedBB axisalignedbb) {
        ...
        /* Allocating ram useless - MineLevel
    ...
    //Gonzalo Ciocca -> Modified TickEntities Method.
    ...
    public void tickEntities() {
        ...
        entityList.removeAll(g);
            ...
            for (Entity entity : g) {
                ...
                j = entity.ae;
                k = entity.ag;
                if (entity.ad && isChunkLoaded(j, k, true)) {
        ...
        for (entityLimiter.initTick();
            ...
            try {
                entity = tickPosition < entityList.size() ? entityList.get(tickPosition) : entityList.get(tickPosition = 0);
            }catch(Exception e){
                tickPosition=0;
                entity = entityList.get(tickPosition=0);
            }
            // CraftBukkit end
            if (entity.vehicle != null) {
            ...
            if (entity.dead) {
                ...
                tickPosition--;// CraftBukkit - Use field for loop variable
                guardEntityList = true; // Spigot
                b(entity);
            }
        ...
        if (!this.b.isEmpty()) {
            for (int l = 0; l < this.b.size(); ++l) {
                ...
                synchronized (b){
                    tileentity1 = (TileEntity) this.b.get(l);
                }
...
synchronized (b){
    ...
    public TileEntity getTileEntity(BlockPosition blockposition,int run) {
run++;
        if (!this.isValidLocation(blockposition)) {
            return null;
        } else {
            ...
            if (tileentity == null) {
                tileentity = this.getChunkAtWorldCoords(blockposition).a(blockposition, Chunk.EnumTileEntityState.IMMEDIATE,run);
            }
    ...
    public void setTileEntity(BlockPosition blockposition, TileEntity tileentity) {
        this.setTileEntity(blockposition,tileentity,0);
    }

    public void setTileEntity(BlockPosition blockposition, TileEntity tileentity,int run) {
        if (tileentity != null && !tileentity.x()) {
            ...
            if (captureBlockStates) {
                ...
                if(run>5){
                    System.out.println("setTileEntity Run > 5 == "+run+" at X:"+blockposition.getX()+" Y:"+blockposition.getY()+" Z:"+blockposition.getZ());
                    this.getChunkAtWorldCoords(blockposition).tileEntities.put(blockposition,tileentity);
                    return;}
                return;
            }
            if(run>5){
            ...
            if (this.M) {
                ...
                while (iterator.hasNext()) {
                    TileEntity tileentity1;
                    synchronized (b){tileentity1 = (TileEntity) iterator.next();}
                    if (tileentity1.getPosition().equals(blockposition)) {
                        if(tileentity1 instanceof TileEntityChest){
                            ((TileEntityChest) tileentity1).y(run);
                            iterator.remove();
                        }else{
                        tileentity1.y();
                        iterator.remove();
                ...
                synchronized (b){this.b.add(tileentity);}
            } else {
                this.a(tileentity);
    ...
    public void t(BlockPosition blockposition) {
        TileEntity tileentity = this.getTileEntity(blockposition,0);


            if (tileentity != null && this.M) {
                tileentity.y();synchronized (b){
    ...
    protected void D() {
        ...
        if (spigotConfig.randomLightUpdates && !this.players.isEmpty()) { // Spigot
            ...
            EntityHuman entityhuman = (EntityHuman) this.players.get(this.random.nextInt(this.players.size()));
    ...
    public CrashReportSystemDetails a(CrashReport crashreport) {
        ...
        crashreportsystemdetails.a("Chunk stats", new Callable() {
            ...
            public Object call() {
                try{
                return this.a();}catch(Exception e){e.printStackTrace();}
                return null;
            }
        });
